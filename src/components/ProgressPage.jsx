  ...
  return (sets || []).map(function (st, i) {
    ...
    return (
      <div key={keyPrefix + "-" + i} className={cx(s.setRow, isBest && s.setRowBest)}>
        ...
        <span className={cx(s.setRowE1rm, isBest && s.setRowE1rmBest)}>
          {setE1 != null ? (isBest ? "Best e1RM " + setE1 : "e1RM " + setE1) : "—"}
        </span>
      </div>
    );
  });
...
function buildSetChartData(sets) {
  return (sets || []).map(function (st, i) {
    ...
    return {
      ...
      failed: failed,
      attemptWeight: wt,
      weight: !failed && wt > 0 ? wt : null,
      volume: !failed && vol > 0 ? Math.round(vol) : null,
      e1rm: !failed ? e1 : null,
      reps: rp,
    };
  });
...
function withFailedPlot(data, metric) {
  return data.map(function (p) {
    if (!p.failed) return p;
    var plotY = metric === "weight" ? (p.attemptWeight > 0 ? p.attemptWeight : 0) : 0;
    return Object.assign({}, p, { failedPlot: plotY });
  });
}
...
function FailedSetDot(props) {
  ...
  return (
    <g>
      <line x1={cx - size} y1={cy - size} x2={cx + size} y2={cy + size} stroke="#f87171" strokeWidth={2.5} strokeLinecap="round" />
      <line x1={cx + size} y1={cy - size} x2={cx - size} y2={cy + size} stroke="#f87171" strokeWidth={2.5} strokeLinecap="round" />
    </g>
  );
}

function SessionSetDot(props) {
  var cx = props.cx;
  var cy = props.cy;
  var fill = props.fill;
...
function createSessionSetAxisTick(data) {
  return function SessionSetAxisTick(tickProps) {
    ...
    var labelColor = point.failed ? "#f87171" : "#e2e8f0";
    return (
      <g transform={"translate(" + x + "," + y + ")"}>
        <text y={0} dy={12} textAnchor="middle" fill={labelColor} fontSize={10}>{point.label}</text>
...
function SessionSetTooltip({ active, payload, metricLabel }) {
  ...
  if (point.failed) {
    return (
      <div className={s.tooltip} style={{ border: "1px solid #f87171" }}>
        ...
        <div style={{ color: "#f87171", fontWeight: 700, marginBottom: 4 }}>Failed attempt</div>
        <div style={{ color: "var(--ft-text-muted)", fontSize: 11 }}>{point.attemptWeight || 0}kg × 0</div>
  ...
  return (
    <div className={s.tooltip} style={{ border: "1px solid var(--ft-border-input)" }}>
      ...
      {point.reps != null && <div style={{ color: "var(--ft-text-muted)", fontSize: 11, marginTop: 4 }}>{point.attemptWeight || point.weight || 0}kg × {point.reps}</div>}
      {point.time && <div style={{ color: "var(--ft-text-dim)", fontSize: 11, marginTop: 2 }}>{point.time}</div>}
    </div>
  );
}
...
function WorkoutSessionGraph({ workout, colorFallbackIdx, onClose, formatChartDate, selectedDateKey }) {
  ...
  function renderMetricChart(data, strokeColor, height, chartOpts) {
    ...
    var hasTimes = data.some(function (p) { return !!p.time; });
    var hasFailed = data.some(function (p) { return p.failed; });
...
function ExerciseChart({ ex, sessions, colorFallbackIdx, onPointSelect, formatChartDate, getChartDateKey }) {
  ...
  return (
    <div className={s.exerciseCard}>
      ...
      {latest && <div className={s.statStrip}>{[
        { label: "Last Weight", val: latest.weight + " kg", color: exColor },
        { label: "Last Volume", val: String(latest.volume), color: ORANGE },
        { label: "Last Best e1RM", val: latest.e1rm != null ? latest.e1rm + " kg" : "—", color: GREEN },
        { label: "Last Mean e1RM", val: latest.mean_e1rm != null ? latest.mean_e1rm + " kg" : "—", color: ACCENT },
      ].map(function (st) { return <div key={st.label} className={s.miniStat}><div className={s.miniStatLabel}>{st.label}</div><div className={s.miniStatValue} style={{ color: st.color }}>{st.val}</div></div>; })}</div>}
