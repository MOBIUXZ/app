...
function ExerciseChart({ ex, data, compoundIdx, onPointSelect, formatChartDate }) {
  ...
  var chartData = cd.map(function (point) { return Object.assign({}, point, { rawDate: point.date, date: formatChartDate(point.date) }); });
...
export default function ProgressPage({ data }) {
  ...
  var selectedWorkouts = selectedDate ? normalizedWorkouts.filter(function (w) { return String(w.date) === String(selectedDate); }) : [];
  ...
  return (
    <div>
      ...
      {allEx.length === 0 ? <Card><div style={{ color: "#6b7280", fontSize: 13, textAlign: "center", padding: "20px 0" }}>No workouts logged yet.</div></Card> : <div>
        ...
        {compounds.map(function (ex, i) { return <ExerciseChart key={ex} ex={ex} data={normalizedData} compoundIdx={i} onPointSelect={setSelectedDate} formatChartDate={formatChartDate} />; })}
        ...
        {isolations.map(function (ex, i) { return <ExerciseChart key={ex} ex={ex} data={normalizedData} compoundIdx={i} onPointSelect={setSelectedDate} formatChartDate={formatChartDate} />; })}
