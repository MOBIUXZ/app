...
function DashboardPage({data,setTab}){
  ...
  var prKb = useKeyboardListNav(prList.length, function () { setTab("Workout"); }, prList.length > 0);
  var recentKb = useKeyboardListNav(recentList.length, function () { setTab("Workout"); }, recentList.length > 0);
  return (
    <div>
      ...
      <Card>
        ...
        <div ref={prKb.listRef} tabIndex={0} onKeyDown={prKb.handleKeyDown} style={{ outline: "none" }}>
          {prList.map(function(kv, i){return <div key={kv[0]} data-kb-index={i} className={prKb.kbClass(i)} onMouseEnter={function(){prKb.setFocusIdx(i);}} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #2d2d3a",fontSize:14,cursor:"pointer"}} onClick={function(){setTab("Workout");}}><span>{formatExerciseName(kv[0])}</span><span style={{color:ACCENT,fontWeight:700}}>{kv[1]} kg</span></div>;})}
        </div>}
      </Card>
      <Collapse emoji="📋" label="Recent Workouts" defaultOpen={false}>
        <Card>
          ...
          <div ref={recentKb.listRef} tabIndex={0} onKeyDown={recentKb.handleKeyDown} style={{ outline: "none" }}>
            {recentList.map(function(w,i){return <div key={i} data-kb-index={i} className={recentKb.kbClass(i)} onMouseEnter={function(){recentKb.setFocusIdx(i);}} onClick={function(){setTab("Workout");}} style={{padding:"8px 0",borderBottom:"1px solid #2d2d3a",fontSize:13,cursor:"pointer"}}><span style={{color:ACCENT,fontWeight:700}}>{formatExerciseName(w.exercise)}</span><span style={{color:"#6b7280",marginLeft:8,fontSize:12}}>{w.date}{w.time?" · "+w.time:""}</span><div style={{color:"#9ca3af",fontSize:12}}>{w.sets.map(function(s){return s.weight+"kg×"+s.reps;}).join(" • ")}</div></div>;})}
          </div>}
        </Card>
      </Collapse>
    </div>
