...
| Progress muscle/fat overlay | `SMM`/`smm` and `FM`/`fm` | One overlay card just above Visceral Fat; both lines share a Y-axis; hidden when both empty |
...
4. BODY label (extra space + hairline) then footer: Body Weight card, Fat / Muscle / FFM stacked cards, Segmental Analysis card with Soft Lean / Fat / Merged toggle (if any series), Muscle Mass & Fat Mass overlay card (if either series), Visceral Fat Level card (if any series), Water / Protein / Mineral card (if any series), BMR card (if any series), InBody Score card (if any series), Calorie Intake Trend card
