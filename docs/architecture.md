...
| Progress segmental lean/fat charts | `inbody.lean*` / `fat*` per region | Own Y-axis per region; card hidden if empty |
...
4. Footer: Body Weight & BF card, InBody Trends card (if any series), Segmental Lean and Segmental Fat cards (if any series), Calorie Intake Trend card
