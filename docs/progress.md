...
One Progress card after Segmental Analysis and just above Visceral Fat Level. A Muscle & Fat / Muscle - Fat / Muscle : Fat Ratio pill switches views (the pill is hidden when only one view has data). Default view overlays Skeletal Muscle Mass and Fat Mass on one chart (crimson muscle, yellow fat) with a shared Y-axis. The delta view plots `SMM − FM` on a green line in kg, with a horizontal reference line at y=0 and a matching **0** tick on the Y-axis (the Y-axis includes 0). The ratio view plots `SMM / FM` on an accent line. An empty series is omitted; the card is hidden when overlay, delta, and ratio are all empty. Titles, colors, units, and field paths live in `spec/page-layout.json` → `pages.progress.massOverlayTrends`. Series: `buildAllBodyTrendSeries()` + `flattenMassOverlayCharts()` + `buildOverlayTrendModel()` in `src/domain/bodyTrends.js`. Delta uses `deriveSmmFmDelta()` and ratio uses `deriveSmmFmRatio()` in `src/domain/metrics.js`.

| Line | Source | Unit |
|------|--------|------|
| Muscle Mass (kg) | `SMM` / `smm` | kg |
| Fat Mass (kg) | `FM` / `fm` | kg |
| Muscle Mass - Fat Mass | derived `SMM − FM` | kg |
