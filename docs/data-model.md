...
**Import:** Body Comp → History → Import InBody CSV. Merges by date; does not replace workouts or calories. Progress charts SMM, SMI, FM, BMI, visceral fat, InBody Score, BMR, TBW, protein, mineral, and segmental lean/fat by region (`src/domain/bodyTrends.js`). Body Comp History shows a segmental lean/fat map from `inbody.lean*` / `fat*` (`src/domain/bodySegmental.js`). Spec: [`spec/inbody-csv-fixtures.json`](../spec/inbody-csv-fixtures.json).
